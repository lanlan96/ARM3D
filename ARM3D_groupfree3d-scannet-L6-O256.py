...
model = dict(
    bbox_head=dict(
        ...
        relation_module_cfg=dict(
            proposal_pickup_approach='fps',  # FIXED
            geometry_encode_approach='pre+condition',  # FIXED
            context_encode_approach='encoder',  # FIXED
            relation_decode_approach='default',  # FIXED
            anchor_points=15,  # FIXED
            pickup_points=128, # HP
            max_iter=20,  # DIS
            sub_sampling=None,  # DIS
            max_neighbors=5,  # DIS
            conv_chs=(3, 16, 8, 1),  # DIS
            var=125.0,  # DIS
            in_channels=288,  # FIXED
            conv_ratio=(1/2, 1/2, 1),  # FIXED
            dropout_ratio=(0.25, 0.25, 0.25),  # FIXED
            dropout_type='dropout',  # HP
            conv_cfg=dict(type="Conv1d"),  # FIXED
            norm_cfg=dict(type="BN1d"),  # FIXED
            act_cfg=dict(type="ReLU")  # FIXED
        ),
