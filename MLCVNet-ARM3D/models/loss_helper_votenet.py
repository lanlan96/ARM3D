...
def compute_objectness_loss_cluster(end_points):
    ...
    # print("GT_CENTER:",gt_center.shape)
    # print("GT_CENTER:",gt_center.cpu().detach().numpy())
    B = gt_center.shape[0]
...
def compute_box_and_sem_cls_loss(end_points, config):
    ...
    size_residual_label_normalized = size_residual_label / mean_size_label # (B,K,3) #TODO: 看不懂
...
def get_loss_with_rn(end_points, config):
    ...
    # loss = vote_loss + 0.5*objectness_loss + 2*box_loss + 0.5*sem_cls_loss + 0.1*alpha*rn_cls_losses[1] + 0.1*beta*rn_cls_losses[2] #TODO: ori rn loss 0.1
    # loss = vote_loss + 0.5*objectness_loss + box_loss + 0.2*sem_cls_loss + 0.1*alpha*rn_cls_losses[1] + 0.1*beta*rn_cls_losses[2] #TODO: ori rn loss 0.1
    '''ori'''
    loss = vote_loss + 0.5*objectness_loss + box_loss + 0.1*sem_cls_loss + 0.1*alpha*rn_cls_losses[1] + 0.1*beta*rn_cls_losses[2] #TODO: ori rn loss 0.1
    ...
    # medium_dist = objectness_label.shape[1] - torch.sum(objectness_mask, dim=1)
    # print("\nmedium:",medium_dist)
    obj_pred_acc = torch.sum((obj_pred_val==objectness_label.long()).float())/(objectness_label.shape[0]*objectness_label.shape[1])
    print ("loss helper obj:", obj_pred_acc)
    print("objectness_label:", torch.sum(objectness_label,dim=1))
    end_points['obj_acc'] = obj_pred_acc #ori: obj_acc

    return loss, end_points

def get_loss_with_rn_contrastive(end_points, end_points2,  config):
    """ Loss functions
    ...
    loss1 = vote_loss + 0.5*objectness_loss + box_loss + 0.1*sem_cls_loss + 0.1*alpha*rn_cls_losses[1] + 0.1*beta*rn_cls_losses[2] #新权重
    end_points['loss1'] = loss1
    end_points2['loss1'] = loss1
    # print("vote_loss:{},objectness_loss:{},box_loss:{},sem_cls_loss:{},rn_cls_losses:{}".format(vote_loss,objectness_loss,box_loss,sem_cls_loss,rn_cls_losses))
    print("loss1:",loss1)
    # --------------------------------------------
    # Some other statistics
    obj_pred_val = torch.argmax(end_points['objectness_scores'], 2) # B,K
...
###########################################################
    # Vote loss
    vote_loss = compute_vote_loss(end_points2)
    end_points2['vote_loss'] = vote_loss

    # Obj loss
    objectness_loss, objectness_label, objectness_mask, object_assignment = \
        compute_objectness_loss(end_points2)
    end_points2['objectness_loss'] = objectness_loss
    end_points2['objectness_label'] = objectness_label
    end_points2['objectness_mask'] = objectness_mask
    end_points2['object_assignment'] = object_assignment
    total_num_proposal = objectness_label.shape[0]*objectness_label.shape[1]
    end_points2['pos_ratio'] = \
        torch.sum(objectness_label.float().cuda())/float(total_num_proposal)
    end_points2['neg_ratio'] = \
        torch.sum(objectness_mask.float())/float(total_num_proposal) - end_points2['pos_ratio']

    # Box loss and sem cls loss
    center_loss, heading_cls_loss, heading_reg_loss, size_cls_loss, size_reg_loss, sem_cls_loss = \
        compute_box_and_sem_cls_loss(end_points2, config)
    end_points2['center_loss'] = center_loss
    end_points2['heading_cls_loss'] = heading_cls_loss
    end_points2['heading_reg_loss'] = heading_reg_loss
    end_points2['size_cls_loss'] = size_cls_loss
    end_points2['size_reg_loss'] = size_reg_loss
    end_points2['sem_cls_loss'] = sem_cls_loss
    box_loss = center_loss + 0.1*heading_cls_loss + heading_reg_loss + 0.1*size_cls_loss + size_reg_loss
    end_points2['box_loss'] = box_loss

    # Relation cls loss
    rn_cls_losses = compute_multi_rn_cls_loss(end_points2)
    end_points2['rn_cls_loss'] = rn_cls_losses[0]

    obj_pred_val = torch.argmax(end_points2['objectness_scores'], 2) # B,K
    obj_acc = torch.sum((obj_pred_val==objectness_label.long()).float()*objectness_mask)/(torch.sum(objectness_mask)+1e-6)
    end_points2['obj_acc'] = obj_acc

    alpha = 1.0
    beta = 1.0

    print("alpha:{},  beta:{}".format(alpha, beta))
    # loss2 = vote_loss + 0.5*objectness_loss + box_loss + 0.1*sem_cls_loss + 0.1*alpha*rn_cls_losses[1] + 0.1*beta*rn_cls_losses[2]
    loss2 = vote_loss + 0.5*objectness_loss + box_loss + 0.1*sem_cls_loss + 0.1*alpha*rn_cls_losses[1] + 0.1*beta*rn_cls_losses[2]

    end_points['loss2'] = loss2
    end_points2['loss2'] = loss2
    print("loss2:",loss2)
    # print("vote_loss2:{},objectness_loss2:{},box_loss2:{},sem_cls_loss2:{},rn_cls_losses2:{}".format(vote_loss,objectness_loss,box_loss,sem_cls_loss,rn_cls_losses))
    """
    总LOSS loss1无attention, loss2加了attention
    """
    contrastive_loss  = (loss1 -loss2) #大概是1.6-1.5的水平？
    # contrastive_loss = (loss2 - loss) / loss2
    print("contrastive loss:", contrastive_loss)
    # loss -= contrastive_loss * 0.1 #0.05  0.9 * 0.1 =0.1
    loss = loss1 + loss2 - contrastive_loss
    print("LOSS:",loss)
    loss *= 10

    end_points['loss'] = loss
    end_points['contrastive_loss'] = contrastive_loss

    end_points2['loss'] = loss
    end_points2['contrastive_loss'] = contrastive_loss

    return loss, end_points2
...
def compute_multi_rn_cls_loss_cluster(end_points):
    ...
    # _mask_j = _mask_j[torch.arange(bs).cuda().view(bs, -1), torch.arange(bs * roi_num * pair_num).view(bs, -1)]  #(8,256*256)

    rn_mask = (_mask_i.mul(_mask_j)).float()
    
    """
    Attention weight rn_mask
    """
    if 'same_category' in relation_type:
        """
            Compute loss for relation_0: same category
    ...
    if 'support' in relation_type:
        """
            Compute loss for relation_1: support
        """
        ...
        print("compute support cost: ", time.time()-start_time)
...
def compute_multi_rn_cls_loss(end_points):
    ...
    Attention weight rn_mask
    """
    # rn_objectness = end_points['rn_objectness'] 
    # rn_mask = rn_mask * rn_objectness
    # print("rn_mask:",torch.max(rn_mask),torch.min(rn_mask),torch.mean(rn_mask))
    if 'same_category' in relation_type:
        """
            Compute loss for relation_0: same category
