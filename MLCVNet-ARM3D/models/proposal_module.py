...
from rn_module_3DRM import RNModule as RN3DRM
...
class ProposalModule_3DRM(nn.Module):
    def __init__(self, num_class, num_heading_bin, num_size_cluster, mean_size_arr, num_proposal, sampling, seed_feat_dim=256,relation_pair=3, relation_type=['same_category'], random=False):
        super().__init__() 
        ...
        self.rnet = RN3DRM(relation_pair=relation_pair, relation_type=relation_type, random=random)
    ...
    def forward(self, xyz, features, end_points):
        ...
        end_points['aggregated_vote_xyz'] = xyz # (batch_size, num_proposal, 3)
        end_points['aggregated_vote_inds'] = sample_inds # (batch_size, num_proposal,) # should be 0,1,2,...,num_proposal
        # aggregated_features = features.clone()
        feature_dim = features.shape[1]
        batch_size = features.shape[0]
        features = features.contiguous().view(batch_size, feature_dim, 16, 16)
        net = self.sa1(features)
        ...
        # --------- PROPOSAL GENERATION ---------
        # Add rn feature
        # end_points = self.rnet(net2, end_points)
        end_points = self.rnet(net, end_points)
        rn_feature = end_points['rn_feature']
    

        net = torch.cat((net, rn_feature), 1) #256维度吗?进入conv1之前 torch.Size([8, 256, 256])
        net = F.relu(self.bn1(self.conv1(net)))
        net = F.relu(self.bn2(self.conv2(net))) 
        net = self.conv3(net) # (batch_size, 2+3+num_heading_bin*2+num_size_cluster*4, num_proposal)
...
class ProposalModule_pred(nn.Module):
    ...
    def forward(self, xyz, features, end_points):
        ...
        # objectness_pred, end_points = self.onet(net2, end_points)
        objectness_pred, end_points = self.onet(net, end_points)
