...
init_seed = 1
torch.manual_seed(init_seed) # 为CPU设置随机种子
torch.cuda.manual_seed(init_seed) # 为当前GPU设置随机种子
...
# ------------------------------------------------------------------------- GLOBAL CONFIG BEG
FLAGS.relation_pair = 8 #TODO: RN-modified
# FLAGS.relation_type = ['same_category','support'] #TODO: RN-modified
# FLAGS.relation_type = ['support'] #TODO: RN-modified
FLAGS.relation_type = ['same_category'] #TODO: RN-modified
time_string = datetime.now().strftime('%Y-%m-%d-%H-%M-%S')
FLAGS.random = True #TODO: RN-modified
if FLAGS.random:
    ranornear = "_random_"
else:
    ranornear = "_nearest_"
FLAGS.log_dir = FLAGS.log_dir + '/log_rn' + str(FLAGS.relation_pair) + '_' + "_".join(FLAGS.relation_type) + ranornear+time_string+"_GT_restore_8" #TODO: RN-modified
...
net = Detector(num_class=DATASET_CONFIG.num_class,
               ...
               relation_pair=FLAGS.relation_pair,#TODO: RN-modified
               relation_type=FLAGS.relation_type,#TODO: RN-modified
               random=FLAGS.random)#TODO: RN-modified
...
net.to(device)
criterion = MODEL.get_loss_with_rn   #TODO: RN-modified
...
if CHECKPOINT_PATH is not None and os.path.isfile(CHECKPOINT_PATH):
    if FLAGS.obj_restore == 1:
        ...
        #如果checkpoint是net的子集，我觉得应该会比较智能的加载checkpoint有的那部分网络参数吧，没有的网络就初始化了
...
if FLAGS.obj_restore:
    #设置Net哪些部分不需要梯度反传
    for k, v in net.named_parameters():
        if k.startswith("pnet.onet"):
...
bnm_scheduler = BNMomentumScheduler(net, bn_lambda=bn_lbmd, last_epoch=start_epoch-1)   #TODO: 调研一下
...
def train_one_epoch():
    ...
    adjust_learning_rate(optimizer, EPOCH_CNT) #TODO:哪里用到了weight_decay啊
    bnm_scheduler.step() # decay BN momentum
    net.train() # set model to training mode
    for batch_idx, batch_data_label in enumerate(TRAIN_DATALOADER):
        for key in batch_data_label: #DICT's key,不可以直接整个放到cuda吗
            batch_data_label[key] = batch_data_label[key].to(device)

        # Forward pass
        optimizer.zero_grad()
        # inputs = {'point_clouds': batch_data_label['point_clouds']} #ORI
        inputs = {'point_clouds': batch_data_label['point_clouds'],'center_label':batch_data_label['center_label']}
        end_points = net(inputs)

        end_points['relation_type'] = FLAGS.relation_type #TODO: RN-modified
        end_points['relation_pair'] = FLAGS.relation_pair #TODO: RN-modified
...
def evaluate_one_epoch():
    ...
    for key in sorted(stat_dict.keys()):
        log_string('eval mean %s: %f'%(key, stat_dict[key]/(float(batch_idx+1))))   #整个epoch走完，batch_id就变成了最大的
...
def train(start_epoch):
    ...
    for epoch in range(start_epoch, MAX_EPOCH):
        ...
        log_string('Current BN decay momentum: %f'%(bnm_scheduler.lmbd(bnm_scheduler.last_epoch))) #这个不太懂
        log_string(str(datetime.now()))
        # Reset numpy seed.
        # REF: https://github.com/pytorch/pytorch/issues/5059
        np.random.seed()
        train_one_epoch()
        if EPOCH_CNT == 0 or EPOCH_CNT % 10 == 9 or EPOCH_CNT>= MAX_EPOCH-10: # Eval every 10 epochs
        ...
        if mAP > best_mAP:
            print("BEST EPOACH:", EPOCH_CNT)
            log_string("BEST EPOACH: %03d" % (EPOCH_CNT))
            best_mAP = mAP
            torch.save(save_dict, os.path.join(LOG_DIR, 'best_checkpoint.tar'))
            log_string("SAVED best checkpoint")
