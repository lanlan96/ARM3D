...
parser.add_argument('--max_epoch', type=int, default=250, help='Epoch to run [default: 180]')
...
# parser.add_argument('--lr_decay_steps', default='120,160,200', help='When to decay the learning rate (in epochs) [default: 80,120,160]')
parser.add_argument('--lr_decay_steps', default='100,140,180', help='When to decay the learning rate (in epochs) [default: 100,140,180]')
...
FLAGS.relation_pair = 4 #TODO: RN-modified
...
if FLAGS.random:
    ...
    ranornear = "_nearest_"
FLAGS.log_dir = FLAGS.log_dir + '/log_rn' + str(FLAGS.relation_pair) + '_' + "_".join(FLAGS.relation_type) + ranornear+time_string+"_3DRM_both" #TODO: RN-modified
...
def train(start_epoch):
    ...
    for epoch in range(start_epoch, MAX_EPOCH):
        ...
        if EPOCH_CNT == 219:
            print("219 EPOACH:", EPOCH_CNT)
            log_string("219 EPOACH: %03d" % (EPOCH_CNT))
            best_mAP = mAP
            torch.save(save_dict, os.path.join(LOG_DIR, '219_checkpoint.tar'))
            log_string("SAVED 219 checkpoint")
